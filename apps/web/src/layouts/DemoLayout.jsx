...
export default function DemoLayout() {
  ...
  return (
    <DemoProvider>
      <div className="dvh-screen flex overflow-hidden bg-slate-100 dark:bg-slate-950">
        ...
        <aside className={`fixed inset-y-0 left-0 z-50 flex w-64 transform flex-col border-r border-slate-800 bg-slate-950 text-white transition-transform lg:static ${
          ...
          <div className="flex h-16 items-center justify-between border-b border-slate-800 px-5">
            <Link to="/" className="text-lg font-black tracking-tight">Drive<span className="text-syntix-green">Control</span></Link>
            <button type="button" onClick={() => setSidebarOpen(false)} className="rounded-lg p-2 text-slate-400 hover:bg-slate-800 lg:hidden" aria-label="Cerrar menú">
              <X className="h-5 w-5" />
            </button>
          </div>
          ...
          <nav className="min-h-0 flex-1 space-y-1 overflow-y-auto px-3 py-4">
            {navItems.map((item) => (
              <NavLink key={item.path} to={item.path} onClick={() => setSidebarOpen(false)} className={({ isActive }) => `flex min-h-11 items-center gap-3 rounded-xl px-3 py-2 text-sm font-semibold transition ${
                isActive ? 'bg-syntix-green text-white' : 'text-slate-300 hover:bg-slate-800 hover:text-white'
              }`}>
        ...
        <div className="flex min-w-0 flex-1 flex-col overflow-hidden">
          <header className="flex h-16 shrink-0 items-center justify-between border-b border-slate-800 bg-syntix-navy px-4 text-white lg:px-6">
            <button type="button" onClick={() => setSidebarOpen(true)} className="rounded-lg p-2 hover:bg-white/10 lg:hidden" aria-label="Abrir navegación demo"><Menu className="h-5 w-5" /></button>
            ...
            <Link to="/#contacto" className="rounded-xl bg-white px-4 py-2 text-sm font-black text-syntix-navy hover:bg-slate-100">Solicitar acceso</Link>
          </header>
          <DemoBanner />
