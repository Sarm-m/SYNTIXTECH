...
export default function DetallesValidacionModal({
  ...
  const soatExpiration = data
    ? getExpirationAlertText(data.soat.diasRestantes, data.soat.fechaVencimiento)
    : null;
  const rtmExpiration = data
    ? getExpirationAlertText(data.rtm.diasRestantes, data.rtm.fechaVencimiento)
    : null;
