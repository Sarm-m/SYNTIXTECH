...
export default function ReportesPage() {
  ...
  const handleExportCSV = () => {
    ...
    const rows = vehiculos.map((vehiculo) => {
      ...
      const soatStatus = vehiculo.soat?.estado ? getStatusLabel(vehiculo.soat.estado) : 'Sin SOAT';
      const rtmStatus = vehiculo.rtm?.estado ? getStatusLabel(vehiculo.rtm.estado) : 'Sin RTM';
      const licenseStatus = vehiculo.conductor?.estado ? getStatusLabel(vehiculo.conductor.estado) : 'Sin conductor';
      ...
      return [
        ...
        getStatusLabel(vehiculo.estadoGeneral),
      ].map(quoteCsv).join(',');
    });
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div data-onboarding="reports-distribution-card" className={`flex flex-col justify-center rounded-2xl border p-8 shadow-sm ${
          ...
          <div className="space-y-4">
            ...
            <StatusBar label={getStatusLabel('rojo')} value={stateStats.rojo} total={stateStats.total} colorClass="bg-syntix-red" textClass="text-syntix-red" isDarkMode={isDarkMode} />
          </div>
        </div>
      </div>
            ...
            return (
              <div key={alert.id} className={`rounded-xl border p-4 ${
                ...
                <div className="flex justify-between gap-3">
                  <div>
                    <p className={`font-bold ${isDarkMode ? 'text-slate-100' : 'text-gray-900'}`}>{statusLabel}</p>
                ...
                <p className={`mt-2 text-xs font-semibold ${isDarkMode ? 'text-slate-500' : 'text-gray-500'}`}>
                  {alert.reason || expiration.fullText}
                </p>
              </div>
            );
          })}
    ...
    <div className={`rounded-xl border p-4 ${
      ...
      {stats && (
        <div className={`mt-3 grid grid-cols-2 gap-2 text-xs ${isDarkMode ? 'text-slate-300' : 'text-gray-600'}`}>
          <span>{getStatusLabel('verde')}: <strong>{stats.vigente}</strong></span>
          <span>{getStatusLabel('amarillo')}: <strong>{stats.proximo}</strong></span>
          <span>{getStatusLabel('rojo')}: <strong>{stats.vencido}</strong></span>
