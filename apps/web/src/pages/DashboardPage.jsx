...
export default function DashboardPage() {
  ...
  const stats = useMemo(
    () => [
      ...
      { label: 'Alertas', value: isLoading ? '...' : (isReady ? totalActiveAlerts : 0), hint: 'Pendientes' },
    ],
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: 12, marginTop: 14 }}>
            ...
            {recentVehicles.length > 0 ? (
              recentVehicles.map((vehicle) => (
                <div key={vehicle.id} style={alertRow(isDarkMode)}>
                  <span style={badge(getStatusLabel(vehicle.estadoGeneral))}>
                    {getStatusLabel(vehicle.estadoGeneral)}
                  </span>
                  <div>
                    <div style={{ fontWeight: 600, color: pageTextColor }}>{vehicle.placa} - {vehicle.marca} {vehicle.modelo}</div>
                    <div style={{ fontSize: 13, color: mutedTextColor, marginTop: 2 }}>
                      {vehicle.tipo}
                    </div>
                    <div style={{ fontSize: 12, color: secondaryTextColor, marginTop: 2 }}>
                      {vehicle.estadoRazon || 'Documentos vigentes'}
                    </div>
                  </div>
                </div>
              ))
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginTop: 8 }}>
            {alerts.slice(0, 3).map((a) => (
              <div key={a.id} style={alertRow(isDarkMode)}>
                <span style={badge(getStatusLabel(a.prioridad))}>
                  {getStatusLabel(a.prioridad)}
                </span>
                <div>
                  <div style={{ fontWeight: 600, color: pageTextColor }}>{a.mensaje}</div>
                  <div style={{ fontSize: 13, color: mutedTextColor, marginTop: 2 }}>{a.tipo}: {a.entidad}</div>
                  <div style={{ fontSize: 12, color: secondaryTextColor, marginTop: 2 }}>{a.reason}</div>
                </div>
              </div>
            ))}
          </div>
        ...
        <div data-onboarding="dashboard-doc-status" style={card(isDarkMode)}>
          ...
          <div style={{ marginTop: 10, display: 'flex', gap: 10, flexWrap: 'wrap' }}>
            <span style={pill('#e8f5e9', '#2e7d32')}>{getStatusLabel('verde')}</span>
            <span style={pill('#fff8e1', '#a15c00')}>{getStatusLabel('amarillo')}</span>
            <span style={pill('#ffebee', '#b71c1c')}>{getStatusLabel('rojo')}</span>
          </div>
...
function badge(level) {
  const map = {
    ...
    'Por vencer': { bg: '#fff8e1', fg: '#a15c00' },
    'Al d\u00eda': { bg: '#e8f5e9', fg: '#2e7d32' },
