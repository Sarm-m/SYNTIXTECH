...
export const ENABLE_LOCAL_AUTH_FALLBACK =
  Boolean(import.meta.env.DEV && import.meta.env.VITE_ENABLE_LOCAL_AUTH_FALLBACK === 'true');
...
api.interceptors.response.use(
  (response) => response,
  (error) => {
    if (ENABLE_LOCAL_AUTH_FALLBACK && (error.code === 'ECONNREFUSED' || error.code === 'ERR_NETWORK')) {
...
export const authService = {
  ...
  async getMe() {
    try {
      ...
      return {
        success: false,
        message: normalizeApiErrorMessage(error, 'No se pudo cargar el perfil'),
      };
    }
  },
  ...
  async requestEmailChange(newEmail, currentPassword) {
    try {
      ...
      return {
        success: false,
        message: normalizeApiErrorMessage(error, 'No se pudo solicitar el cambio de correo'),
      };
    }
  },
  ...
  async verifyEmailChange(newEmail, code) {
    try {
      const response = await api.post('/auth/email-change/verify', { newEmail, code });
      return {
        success: true,
        data: response.data.data,
        message: response.data.message,
      ...
      return {
        success: false,
        message: normalizeApiErrorMessage(error, 'No se pudo verificar el codigo'),
      };
    }
  },
};
